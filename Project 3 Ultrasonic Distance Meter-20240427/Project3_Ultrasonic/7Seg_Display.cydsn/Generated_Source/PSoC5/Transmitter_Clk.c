...
/*******************************************************************************
* Function Name: Transmitter_Clk_Start
********************************************************************************
*
* Summary:
...
void Transmitter_Clk_Start(void) 
{
    ...
    Transmitter_Clk_CLKEN |= Transmitter_Clk_CLKEN_MASK;
	Transmitter_Clk_CLKSTBY |= Transmitter_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Transmitter_Clk_Stop
********************************************************************************
*
* Summary:
...
void Transmitter_Clk_Stop(void) 
{
    ...
    Transmitter_Clk_CLKEN &= (uint8)(~Transmitter_Clk_CLKEN_MASK);
	Transmitter_Clk_CLKSTBY &= (uint8)(~Transmitter_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Transmitter_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Transmitter_Clk_CLKEN & Transmitter_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Transmitter_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Transmitter_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Transmitter_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Transmitter_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Transmitter_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Transmitter_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Transmitter_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Transmitter_Clk_GetSourceRegister();
    uint16 oldDivider = Transmitter_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Transmitter_Clk_CLKEN & Transmitter_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Transmitter_Clk_DIV_PTR, clkDivider);
                Transmitter_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Transmitter_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Transmitter_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Transmitter_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Transmitter_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Transmitter_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Transmitter_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Transmitter_Clk_MOD_SRC |= modeBitMask & (uint8)Transmitter_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Transmitter_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Transmitter_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Transmitter_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Transmitter_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Transmitter_Clk_MOD_SRC & (uint8)(Transmitter_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Transmitter_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Transmitter_Clk_GetDividerRegister();
    uint8 oldSrc = Transmitter_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Transmitter_Clk_MOD_SRC |= CYCLK_SSS;
        Transmitter_Clk_MOD_SRC =
            (Transmitter_Clk_MOD_SRC & (uint8)(~Transmitter_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Transmitter_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Transmitter_Clk_MOD_SRC & Transmitter_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Transmitter_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Transmitter_Clk_PHASE = clkPhase & Transmitter_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Transmitter_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Transmitter_Clk_PHASE & Transmitter_Clk_PHASE_MASK;
}
